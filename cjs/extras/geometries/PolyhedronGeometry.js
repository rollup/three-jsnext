...
function THREE$PolyhedronGeometry(vertices, indices, radius, detail) {
	...
	for (var i = 0, l = vertices.length; i < l; i += 3) {
		...
		prepare(new _mathVector3.THREE$Vector3(vertices[i], vertices[i + 1], vertices[i + 2]));
	}
	...
	for (var i = 0, j = 0, l = indices.length; i < l; i += 3, j++) {
		...
		faces[j] = new _coreFace3.THREE$Face3(v1.index, v2.index, v3.index, [v1.clone(), v2.clone(), v3.clone()], undefined, j);
	}

	var centroid = new _mathVector3.THREE$Vector3();
	...
	function prepare(vector) {
		...
		vertex.uv = new _mathVector2.THREE$Vector2(u, 1 - v);
	...
	function make(v1, v2, v3, materialIndex) {
		...
		var face = new _coreFace3.THREE$Face3(v1.index, v2.index, v3.index, [v1.clone(), v2.clone(), v3.clone()], undefined, materialIndex);
	...
	function correctUV(uv, vector, azimuth) {
		...
		if (azimuth < 0 && uv.x === 1) uv = new _mathVector2.THREE$Vector2(uv.x - 1, uv.y);
		if (vector.x === 0 && vector.z === 0) uv = new _mathVector2.THREE$Vector2(azimuth / 2 / Math.PI + 0.5, uv.y);
		return uv.clone();
	}
};

THREE$PolyhedronGeometry.prototype = Object.create(_coreGeometry.THREE$Geometry.prototype);
...
THREE$PolyhedronGeometry.prototype.copy = function (source) {
	...
	_coreGeometry.THREE$Geometry.prototype.copy.call(this, source);
	return this;
};
