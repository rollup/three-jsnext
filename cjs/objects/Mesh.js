...
var _mathTriangle = require('../math/Triangle');
...
function THREE$Mesh(geometry, material) {
	...
	this.material = material !== undefined ? material : new _materialsMeshBasicMaterial.THREE$MeshBasicMaterial({ color: Math.random() * 0xffffff });

	this.updateMorphTargets();
};

THREE$Mesh.prototype = Object.create(_coreObject3D.THREE$Object3D.prototype);
...
THREE$Mesh.prototype.raycast = (function () {
	...
	var vB = new _mathVector3.THREE$Vector3();
	var vC = new _mathVector3.THREE$Vector3();

	var tempA = new _mathVector3.THREE$Vector3();
	var tempB = new _mathVector3.THREE$Vector3();
	var tempC = new _mathVector3.THREE$Vector3();
	...
	return function raycast(raycaster, intersects) {
		...
		if (geometry && geometry.isBufferGeometry) {
			...
			if (attributes.index !== undefined) {
				...
				for (var oi = 0, ol = offsets.length; oi < ol; ++oi) {
					...
					for (var i = start, il = start + count; i < il; i += 3) {
						...
						if (material.side === _Three.THREE$BackSide) {

							var intersectionPoint = ray.intersectTriangle(vC, vB, vA, true);
						} else {

							var intersectionPoint = ray.intersectTriangle(vA, vB, vC, material.side !== _Three.THREE$DoubleSide);
						}
						...
						intersects.push({
							...
							face: new _coreFace3.THREE$Face3(a, b, c, _mathTriangle.THREE$Triangle.normal(vA, vB, vC)),
			...
			for (var f = 0, fl = faces.length; f < fl; f++) {
				...
				if (faceMaterial.side === _Three.THREE$BackSide) {

					var intersectionPoint = ray.intersectTriangle(c, b, a, true);
				} else {

					var intersectionPoint = ray.intersectTriangle(a, b, c, faceMaterial.side !== _Three.THREE$DoubleSide);
				}
