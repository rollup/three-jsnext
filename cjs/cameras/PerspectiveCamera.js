...
THREE$PerspectiveCamera.prototype = Object.create(_Camera.THREE$Camera.prototype);
...
THREE$PerspectiveCamera.prototype.setLens = function (focalLength, frameHeight) {
	...
	this.fov = 2 * _mathMath.THREE$Math.radToDeg(Math.atan(frameHeight / (focalLength * 2)));
	this.updateProjectionMatrix();
};
...
THREE$PerspectiveCamera.prototype.updateProjectionMatrix = function () {
	...
	var fov = _mathMath.THREE$Math.radToDeg(2 * Math.atan(Math.tan(_mathMath.THREE$Math.degToRad(this.fov) * 0.5) / this.zoom));

	if (this.fullWidth) {

		var aspect = this.fullWidth / this.fullHeight;
		var top = Math.tan(_mathMath.THREE$Math.degToRad(fov * 0.5)) * this.near;
		var bottom = -top;
